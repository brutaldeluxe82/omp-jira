...
export default function jiraExtension(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		parameters: z.object({
			...
			issueType: z.string().optional().describe("Issue type for issue_create; defaults to Task"),
			summary: z.string().optional().describe("Issue summary for create or update"),
			description: z.string().optional().describe("Plain-text description converted to an Atlassian Document Format document"),
			...
			comment: z.string().optional().describe("Plain-text comment converted to an Atlassian Document Format document"),
